...
import {checkAnswer,getWork,makeQuiz,finishQuiz,quizScore,quizProgress,defaultStore,recordRecall,needsPractice,sanitizeStore,hasPhrase,STUDY_FIELDS,DETAIL_FIELDS,studyFields,FACT_FIELDS,makeChoices,makeStudyQuestions,dateIntervals,dateInRange,choiceAnswer,hasKnownArtist,quizFactFields} from '../dist/engine.js';
...
test('all eligible multiple-choice combinations have four unique choices and one accepted answer',()=>{
...
test('all-details multiple choice covers every work and category',()=>{const qs=makeStudyQuestions({focus:'all',mode:'choice'});assert.equal(qs.length,71);assert.equal(new Set(qs.map(q=>`${q.id}:${q.field}`)).size,71);for(const w of WORKS)assert.deepEqual(qs.filter(q=>q.id===w.id).map(q=>q.field),studyFields('all',w));});
test('Details covers every category except name and material in both modes',()=>{
 assert.deepEqual(studyFields('details'),['date','location','artist','culture']);
 const choices=makeStudyQuestions({focus:'details',mode:'choice'});
 assert.equal(choices.length,43);
 assert.equal(new Set(choices.map(q=>`${q.id}:${q.field}`)).size,43);
...
test('Context is removed and artists are asked only for named makers in every study mode',()=>{
 assert.equal(Object.hasOwn(FIELDS,'context'),false);
 for(const mode of ['type','choice']){
  assert.deepEqual(makeStudyQuestions({focus:'artist',mode}).map(q=>q.id),[21]);
  ...
  for(const focus of ['all','details'])for(const q of makeStudyQuestions({focus,mode})){
   ...
   assert.ok(q.field!=='artist'||hasKnownArtist(getWork(q.id)));
   assert.equal(studyFields(focus,getWork(12)).includes('artist'),false);
   assert.equal(studyFields(focus,getWork(21)).includes('artist'),true);
  }
 }
...
test('quizzes never select unknown artists or offer them as the third-answer category',()=>{
 for(const mode of ['type','choice']){const q=makeQuiz({mode,factField:'artist'});for(const question of q.questions){const named=question.id===21;assert.equal(q.answers[question.id].factField,named?'artist':'date');assert.equal(quizFactFields(getWork(question.id)).includes('artist'),named);if(mode==='choice')assert.equal(Object.hasOwn(question.choices,'artist'),named);}}
});
